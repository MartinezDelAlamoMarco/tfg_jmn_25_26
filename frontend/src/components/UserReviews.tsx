  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold">Valoraciones ({reviews.length})</h3>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {reviews.length === 0 && <p className="text-zinc-400">Sin valoraciones todavía.</p>}
          {reviews.map((r) => (
            <div key={r.id} className="bg-zinc-900 p-4 rounded-lg border border-zinc-700">
              <div className="flex justify-between items-center mb-2">
                <div className="font-bold">{r.author?.name || "Usuario"}</div>
      ...
      {canReview && (
        <form onSubmit={submitReview} className="mt-6 bg-zinc-800 p-4 rounded-lg border border-zinc-700">
          <h4 className="font-bold mb-2">Escribe una valoración</h4>
      ...
      {!canReview && (
        <p className="text-sm text-zinc-500 mt-4 italic">Solo puedes valorar a usuarios con los que hayas realizado una compra/venta.</p>
      )}
    </div>
  );
}
