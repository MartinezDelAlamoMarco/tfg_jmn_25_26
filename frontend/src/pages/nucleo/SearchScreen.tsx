      ...
      axios
        ...
        .then((res) => {
          const matched = res.data.filter((ad: any) => {
            const searchString =
              `${ad.brand_name} ${ad.model_name} ${ad.description} ${ad.province_name} ${ad.year}`.toLowerCase();
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((v) => {
              ...
              const isRent =
                v.is_rent === true || v.is_rent === 1 || v.is_rent === "1";

              return (
                <div
                  key={v.id}
                  className="bg-zinc-800 rounded-xl p-6 border border-zinc-700 flex flex-col hover:border-red-700 hover:shadow-2xl transition group relative"
                >
                  ...
                  <div className="h-48 bg-zinc-900 rounded-lg mb-4 overflow-hidden relative border border-zinc-700">
                    ...
                    <div
                      className={`absolute top-2 left-2 z-10 px-3 py-1 rounded text-[10px] font-black uppercase shadow-lg tracking-widest transition-all ${
                        isRent
                          ? "bg-red-700 text-white border-transparent"
                          : "bg-zinc-900 text-white border-zinc-700 dark:bg-zinc-100 dark:text-zinc-950 dark:border-zinc-300 border backdrop-blur-sm"
                      }`}
                    >
                      {isRent
                    ...
                    {imageUrl ? (
                      ...
                      <div className="flex h-full items-center justify-center text-zinc-600 text-xs font-bold uppercase">
                  ...
                  <div className="flex flex-col gap-1 mb-6 mt-4">
                    <p className="text-xs text-zinc-400">
                      {v.year} • {v.km} km • {v.province_name}
                    </p>
                    <div className="flex items-baseline gap-1">
                      ...
                      {isRent && (
                        <span className="text-sm text-zinc-500 font-bold uppercase italic">
                          {" "}
                  ...
                  <Link
                    ...
                    className="block w-full text-center py-3 bg-red-700 hover:bg-red-600 rounded-lg font-bold uppercase text-sm tracking-widest transition"
                  >
                    {isRent
