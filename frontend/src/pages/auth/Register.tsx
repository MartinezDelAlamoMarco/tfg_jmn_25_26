...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('register.error_register', 'Error en registro'))
    }
  }
      ...
      <div className="max-w-md w-full">
        <div className="bg-zinc-800 rounded-2xl shadow-2xl p-8 border border-zinc-700">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">{t('register.create_account', 'Crear cuenta')}</h1>
            <p className="text-zinc-400">{t('register.join', 'Únete a')} {APP_NAME}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder={t('register.first_name_placeholder', 'Juan')}
                  required
                />
              </div>
              <div>
                <label htmlFor="lastName" className="block text-sm font-medium text-zinc-300 mb-2">
                  {t('register.last_name', 'Apellido')}
                </label>
                <input
                  type="text"
                  id="lastName"
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-3 bg-zinc-700 border border-zinc-600 rounded-lg text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition duration-200"
                  placeholder={t('register.last_name_placeholder', 'Pérez')}
                  required
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full px-4 py-3 bg-zinc-700 border border-zinc-600 rounded-lg text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition duration-200"
                placeholder={t('register.email_placeholder', 'tu@email.com')}
                required
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  aria-label={showPassword ? t('register.hide_password', 'Ocultar contraseña') : t('register.show_password', 'Ver contraseña')}
                >
              ...
              <div className="mt-3 text-sm space-y-1">
                <p className={`${passwordValidations.length ? 'text-green-500' : 'text-zinc-500'} transition-colors duration-200 flex items-center gap-2`}>
                  {passwordValidations.length ? '✓' : '○'} {t('register.min_length', 'Mínimo 8 caracteres')}
                </p>
                <p className={`${passwordValidations.upper ? 'text-green-500' : 'text-zinc-500'} transition-colors duration-200 flex items-center gap-2`}>
                  {passwordValidations.upper ? '✓' : '○'} {t('register.uppercase', 'Al menos una mayúscula')}
                </p>
                <p className={`${passwordValidations.lower ? 'text-green-500' : 'text-zinc-500'} transition-colors duration-200 flex items-center gap-2`}>
                  {passwordValidations.lower ? '✓' : '○'} {t('register.lowercase', 'Al menos una minúscula')}
                </p>
                <p className={`${passwordValidations.number ? 'text-green-500' : 'text-zinc-500'} transition-colors duration-200 flex items-center gap-2`}>
                  {passwordValidations.number ? '✓' : '○'} {t('register.number', 'Al menos un número')}
                </p>
                <p className={`${passwordValidations.special ? 'text-green-500' : 'text-zinc-500'} transition-colors duration-200 flex items-center gap-2`}>
                  {passwordValidations.special ? '✓' : '○'} {t('register.special', 'Al menos un carácter especial')}
                </p>
              </div>
            </div>
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-zinc-300 mb-2">
                {t('register.confirm_password', 'Confirmar contraseña')}
              </label>
              <div className="relative">
                <input
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-white transition duration-150"
                  aria-label={showConfirmPassword ? t('register.hide_password', 'Ocultar contraseña') : t('register.show_password', 'Ver contraseña')}
                >
                  {showConfirmPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>
              {formData.confirmPassword.length > 0 && !passwordsMatch && (
                <p className="text-red-500 text-sm mt-2">{t('register.passwords_not_match', 'Las contraseñas no coinciden')}</p>
              )}
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-zinc-400">
              {t('register.already_have_account', '¿Ya tienes cuenta?')}{' '}
