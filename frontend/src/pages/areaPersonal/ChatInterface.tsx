...
const ChatInterface: React.FC = () => {
  ...
  const cancelRent = async (rentId: number) => {
    if (
      !window.confirm(
        t(
          "chat.cancel_rent_confirm",
          "¿Estás seguro de que quieres cancelar esta reserva de alquiler? Las fechas volverán a estar disponibles.",
        ),
      )
    )
      return;
    ...
    if (
      ...
      !window.confirm(
        t(
          ...
          "¿Seguro que quieres eliminar este chat permanentemente?",
        ),
      )
    )
      return;
  ...
  const handleReserve = async () => {
    if (
      !activeChatId ||
      !window.confirm(
        t("chat.reserve_confirm", "¿Reservar vehículo para este comprador?"),
      )
    )
      return;
  ...
  const handleCancelReserve = async () => {
    if (
      ...
      !window.confirm(
        t(
          "chat.cancel_reserve_confirm",
          "¿Cancelar la reserva y volver a ponerlo disponible?",
        ),
      )
    )
      return;
  ...
  const handleConfirmSale = async () => {
    if (
      ...
      !window.confirm(
        t(
          "chat.confirm_sale_confirm",
          "¿Confirmar venta final? Esto cerrará el resto de chats.",
        ),
      )
    )
      return;
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      setHasReviewed(true);
      alert(
        t("chat.review_sent", "¡Valoración enviada! El chat se cerrará ahora."),
      );
  ...
  return (
    ...
    <div className="fixed top-16 inset-x-0 bottom-0 flex bg-zinc-950 text-zinc-100 overflow-hidden font-sans z-40">
      ...
      <div
        className={`${activeChatId ? "hidden md:flex" : "flex"} w-full md:w-96 shrink-0 flex-col border-r border-zinc-800 bg-zinc-900`}
      >
        <div className="p-4 bg-zinc-900/80 border-b border-zinc-800">
          <h2 className="text-3xl font-black italic uppercase mb-4 text-white">
            {t("chat.messages_title", "Mensajes")}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingList ? (
            <div className="p-8 text-center text-zinc-500 text-[15px] animate-pulse italic">
              {t("chat.loading_chats", "Cargando chats...")}
            </div>
          ) : (
            filteredChats.map((chat) => {
              const otherName =
                String(currentUserId) === String(chat.seller_id)
                  ? chat.buyer_name
                  : chat.seller_name;
              const unread =
                chat.id === activeChatId ? 0 : chat.unread_count || 0;
              return (
                <div
                  key={chat.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <h3 className="font-bold truncate text-[17px]">
                        {otherName}
                      </h3>
                    </div>
      ...
      {/* ÁREA DE CHAT */}
      <div
        className={`${!activeChatId ? "hidden md:flex" : "flex"} flex-1 flex-col bg-zinc-950 relative`}
      >
        {activeChatId && !loadingChat ? (
          <div className="flex flex-col h-full relative">
            ...
            <div className="bg-zinc-900/80 px-3 md:px-5 py-4 border-b border-zinc-800 flex items-center justify-between z-20 shadow-md shrink-0">
              <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
                <button
                  ...
                  className="md:hidden text-zinc-400 p-1 -ml-1 hover:text-white transition-colors shrink-0"
                >
                  <ArrowLeft size={24} />
                </button>
                <div className="h-12 w-12 shrink-0 rounded-full bg-red-600 flex items-center justify-center text-lg font-bold text-white shadow-md">
                  {(String(currentUserId) === String(activeChatData?.seller_id)
                    ? activeChatData?.buyer_name
                    : activeChatData?.seller_name
                  )
                    ?.charAt(0)
                    .toUpperCase()}
                </div>
                <div className="min-w-0 pr-2">
                  ...
                  <p className="text-[12px] md:text-[13px] text-zinc-400 truncate mt-0.5">
                    {activeChatData?.is_rent
                      ? t("chat.rent_of", "Alquiler de: ")
                      : t("chat.negotiating_for", "Negociando por: ")}
                    <span className="text-zinc-200 font-bold">
                      {activeChatData?.brand_name} {activeChatData?.model_name}
                    </span>
                    <span className="ml-1.5 text-red-500 font-black">
                      {activeChatData?.advertisement_price}€
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 md:gap-3 shrink-0">
                <button
                  ...
                  title={t("chat.delete_chat_title", "Borrar Chat")}
                >
                  <Trash2 size={22} />
                </button>
                <div className="relative">
                      ...
                      {isCurrentUserSeller &&
                        activeChatData?.chat_status !== "disabled" && (
                          <>
                            ...
                            {activeChatData?.is_rent ? (
                              <button
                                onClick={openRentManager}
                                className="w-full px-5 py-3.5 flex items-center gap-3 text-[15px] text-zinc-300 hover:bg-zinc-800 text-left"
                              >
                                <CalendarDays size={18} />{" "}
                                {t("chat.manage_rents", "Gestionar Reservas")}
                              </button>
                            ) : (
                              /* LÓGICA DE VENTA */
                              <>
                                ...
                                {activeChatData?.ad_status === "reservado" && (
                                  <>
                                    <button
                                      onClick={openExtensionMenu}
                                      className="w-full px-5 py-3.5 flex items-center gap-3 text-[15px] text-blue-400 hover:bg-zinc-800 text-left"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="w-full px-5 py-3.5 flex items-center gap-3 text-[15px] text-orange-500 hover:bg-zinc-800 text-left"
                                    >
                                      <X size={18} />{" "}
                                      {t(
                                        "chat.cancel_reserve",
                                ...
                                {activeChatData?.ad_status !== "vendido" && (
                                  <button
                                    ...
                                    className="w-full px-5 py-3.5 flex items-center gap-3 text-[15px] text-green-500 hover:bg-zinc-800 text-left font-bold border-t border-zinc-800/50"
                                  >
                                    <CheckCircle2 size={18} />{" "}
                                    {t(
                                      "chat.confirm_final_sale",
              ...
              <div className="absolute inset-0 z-60 bg-black/90 p-6 animate-in slide-in-from-top duration-300">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-black uppercase italic text-white flex items-center gap-2">
                    ...
                    {t("chat.rents_of", "Reservas de")}{" "}
                    {activeChatData?.buyer_name}
                  </h3>
                  <button
                    onClick={() => setShowRentManager(false)}
                ...
                <div className="space-y-4 overflow-y-auto max-h-[80%]">
                  {loadingRents ? (
                    <div className="text-center p-12 text-zinc-500 italic animate-pulse">
                      {t("chat.loading_rents", "Cargando reservas...")}
                    </div>
                  ) : activeUserRents.length === 0 ? (
                    <div className="text-center p-12 bg-zinc-900 rounded-2xl text-zinc-500 font-bold border border-zinc-800 italic">
                      {t(
                        "chat.no_active_rents",
                    ...
                    activeUserRents.map((rent) => (
                      <div
                        key={rent.id}
                        className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex justify-between items-center group hover:border-red-900/50 transition"
                      >
                        <div>
                          <p className="text-xs font-black uppercase text-zinc-500 tracking-widest mb-1">
                            {t("chat.reserved_period", "Periodo Reservado")}
                          </p>
                          <p className="text-lg font-bold text-white">
                            {t("chat.from_date", "Del")}{" "}
                          ...
                          <p className="text-red-500 font-black mt-1">
                            {rent.total_price}
                            {t("chat.total_currency", "€ total")}
                          </p>
                        </div>
                        <button
                          onClick={() => cancelRent(rent.id)}
                          className="bg-red-600/10 hover:bg-red-600 text-red-600 hover:text-white p-3 rounded-xl transition flex items-center gap-2 font-bold uppercase text-xs"
                        >
                          <Trash2 size={18} />{" "}
                          {t("chat.cancel_reserve", "Cancelar Reserva")}
                        </button>
                      </div>
                    ))
                  )}
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl shadow-2xl w-full max-w-md">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-black uppercase italic text-white">
                      {t("chat.extend_reserve", "Ampliar Reserva")}
                    </h3>
                    <button
                  ...
                  <p className="text-zinc-400 text-sm mb-4">
                    {t(
                      "chat.extend_reserve_desc",
                      "Selecciona la nueva fecha y hora límite para la reserva de este vehículo.",
                    )}
                  </p>
                  <input
                    type="datetime-local"
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      onClick={handleExtendReserve}
                      className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-500 transition shadow-lg"
                    >
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5 bg-zinc-950"
            >
              {messages.map((msg) => {
                ...
                const isSystem =
                  ...
                  String(msg.sender_id) === "1" ||
                  String(msg.sender_id) === "2";

                if (isSystem) {
                  return (
                    <div
                      key={msg.id}
                      className="flex flex-col items-center my-8 animate-in fade-in zoom-in duration-500"
                    >
                      <span className="text-[11px] text-zinc-500 uppercase font-black tracking-widest mb-2 italic">
                        {t("chat.notification_from", "Notificación de")}{" "}
                        {APP_NAME}
                      </span>
                ...
                return (
                  <div
                    key={msg.id}
                    className={`flex ${isMe ? "justify-end" : "justify-start"} w-full animate-in slide-in-from-bottom-2 duration-300`}
                  >
                    <div
                      className={`max-w-[85%] md:max-w-[75%] px-4 py-3 md:px-5 md:py-3 rounded-2xl ${isMe ? "bg-red-600 text-white rounded-tr-sm shadow-md" : "bg-zinc-800 text-zinc-100 rounded-tl-sm border border-zinc-700 shadow-md"}`}
                    >
                      <p className="text-[16px] md:text-[17px] leading-relaxed whitespace-pre-wrap font-medium wrap-break-words">
                        {msg.content}
                      </p>
            ...
            {/* VALORACIÓN */}
            {activeChatData?.chat_status === "sold" &&
              String(currentUserId) === String(activeChatData?.buyer_id) &&
              !hasReviewed && (
                <div className="mx-4 mb-4 p-6 md:p-8 bg-zinc-800 border-2 border-yellow-600/30 rounded-2xl text-center animate-in zoom-in shadow-2xl relative shrink-0">
                  <button
                    onClick={() => handleFinalizeChat(activeChatId!)}
                    className="absolute top-4 right-4 text-zinc-500 hover:text-white transition-colors"
                  >
                    <X size={24} />
                  </button>
                  <h3 className="text-xl font-black uppercase italic mb-2 text-white">
                    {t("chat.purchase_finished", "¡Compra finalizada!")}
                  </h3>
                  <p className="text-zinc-400 text-[13px] md:text-[14px] mb-5 uppercase tracking-widest px-2">
                    {t("chat.rate_user", "Valora a")}{" "}
                    {activeChatData?.seller_name}{" "}
                    {t("chat.to_help_community", "para ayudar a la comunidad")}
                  </p>
                  <div className="flex justify-center gap-2 md:gap-4 mb-6">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        key={s}
                        onClick={() => setRating(s)}
                        className={`transition-transform active:scale-90 ${rating >= s ? "text-yellow-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.5)]" : "text-zinc-700"}`}
                      >
                        <Star
                          size={36}
                  ...
                  <textarea
                    placeholder={t(
                      "chat.review_placeholder",
                      "Cuéntanos cómo fue todo (opcional)...",
                    )}
                    value={reviewComment}
                  ...
                  <button
                    ...
                    disabled={!rating || loadingReview}
                    className="w-full py-3.5 bg-yellow-600 text-black text-[15px] font-black uppercase tracking-widest rounded-xl transition hover:bg-yellow-500 disabled:opacity-50 shadow-lg"
                  >
                    {loadingReview
                      ? t("common.sending", "Enviando...")
                      : t("chat.submit_review", "Enviar Valoración")}
                  </button>
                </div>
              )}
            ...
            <div className="p-3 md:p-5 bg-zinc-900/80 border-t border-zinc-800 pb-safe shrink-0">
              {activeChatData?.chat_status === "disabled" ? (
                <div className="flex justify-center">
                  <button
                    onClick={handleDeleteChat}
                    className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-4 px-8 rounded-xl border border-zinc-700 transition w-full max-w-md shadow-lg italic tracking-wide uppercase text-xs md:text-sm"
                  >
                    {t(
                      "chat.car_sold_to_other",
                ...
                >
                  ...
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 w-full bg-zinc-900 border border-zinc-700 rounded-2xl px-4 py-3 outline-none text-[16px] md:text-base resize-none focus:border-red-600 transition shadow-inner"
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 p-8 text-center bg-zinc-950">
            ...
            <h3 className="font-black uppercase italic text-zinc-400 text-2xl tracking-tighter">
              {t("chat.mailbox_title", "Buzón de Negociación")}
            </h3>
            <p className="max-w-sm text-[15px] mt-3 leading-relaxed">
              {t(
                "chat.mailbox_desc",
                "Selecciona un chat lateral para empezar a hablar con el vendedor o gestionar tu compra.",
              )}
            </p>
          </div>
        )}
