...
const CreateAd = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const options = {
        maxSizeMB: 1, // Tamaño máximo final (1MB es perfecto para web)
        maxWidthOrHeight: 1920, // Resolución Full HD
        useWebWorker: true,
      };

      try {
        // Mostramos un mensaje de "Procesando..." si quieres
    ...
    <div className="min-h-screen bg-zinc-900 text-white p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <form
          ...
          className="space-y-8 bg-zinc-800 p-6 md:p-10 rounded-3xl border border-zinc-700 shadow-2xl"
        >
          <section className="space-y-6">
            <h2 className="text-red-500 font-bold uppercase text-xs tracking-widest border-l-4 border-red-600 pl-3">
              Datos Técnicos
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <select
                ...
                className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-red-600 transition"
              >
              ...
              <select
                ...
                className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none disabled:opacity-30 focus:ring-2 focus:ring-red-600 transition"
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <select
                ...
                onChange={handleChange}
                className="bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-red-600"
              >
              ...
              <input
                ...
                className="bg-zinc-900 border border-zinc-700 rounded-xl p-3 focus:ring-2 focus:ring-red-600 outline-none"
              />
              <input
                type="text"
                name="hp"
            ...
            <div className="grid grid-cols-2 gap-4">
              <select
                name="tonality_id"
                required
                value={formData.tonality_id}
                onChange={handleChange}
                className="bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-red-600"
              >
                <option value="">Color / Tonalidad</option>
                {tonalities.map((ton: any) => (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {previews.map((preview, index) => (
                    <div key={index} className="relative group">
                      <img
                        src={preview}
                        alt={`Foto ${index + 1}`}
                        className="rounded-xl object-cover w-full h-32 shadow-2xl border border-zinc-700"
                      />
                      ...
                      <button
                        ...
                        className="absolute -top-2 -right-2 bg-red-600 rounded-full w-7 h-7 font-bold border-2 border-zinc-800 flex items-center justify-center hover:bg-red-500 transition-colors opacity-100 md:opacity-0 md:group-hover:opacity-100 z-10"
                      >
                        ✕
                      </button>
                  ...
                  {selectedFiles.length < 5 && (
                    <label className="cursor-pointer border-2 border-dashed border-zinc-600 rounded-xl flex flex-col items-center justify-center h-32 hover:bg-zinc-800 transition group">
                      ...
                      <span className="text-xs text-zinc-500 group-hover:text-white transition-colors mt-1">
                        Añadir más
                      </span>
                      <input
                        type="file"
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <input
                ...
                onChange={handleChange}
                placeholder="Título (Ej: BMW M4 Competition impecable...)"
                className="md:col-span-2 bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-red-600"
              />
              <input
                type="text"
                name="price"
                ...
                onChange={handleNumericChange}
                placeholder="Precio (€)"
                className="bg-zinc-900 border border-zinc-700 rounded-xl p-3 outline-none text-red-500 font-bold focus:ring-2 focus:ring-red-600"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input
            ...
            <textarea
              ...
              onChange={handleChange}
              placeholder="Describe el estado del vehículo, extras, mantenimientos..."
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-red-700 hover:bg-red-600 py-5 rounded-2xl font-black uppercase tracking-widest transition-all shadow-lg shadow-red-900/30 active:scale-95 disabled:opacity-50 flex justify-center items-center gap-3"
          >
            {loading ? (
              <>
                <svg
