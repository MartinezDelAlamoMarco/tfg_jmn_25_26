...
const AdvertisementsScreen = () => {
  ...
  // NUEVO: Filtramos los anuncios dependiendo de la pestaña activa y la columna is_rent
  const filteredAds = myAds.filter((ad: any) => 
    activeTab === 'venta' ? !ad.is_rent : ad.is_rent
  );
    ...
    <div className="min-h-screen bg-zinc-900 p-8 text-white">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-zinc-800 pb-4 gap-4 md:gap-0">
          <div>
            ...
            <div className="flex gap-2 md:gap-4">
              <button 
                ...
                className={`px-4 md:px-6 py-2 rounded-t-lg font-bold uppercase text-xs md:text-sm transition-colors ${
                  activeTab === 'venta' ? 'bg-red-700 text-white' : 'bg-zinc-800 text-zinc-500 hover:text-white'
                }`}
              >
                Venta
              </button>
              <button 
                ...
                className={`px-4 md:px-6 py-2 rounded-t-lg font-bold uppercase text-xs md:text-sm transition-colors ${
                  activeTab === 'alquiler' ? 'bg-red-700 text-white' : 'bg-zinc-800 text-zinc-500 hover:text-white'
                }`}
              >
                Alquiler
              </button>
          ...
          {/* BOTÓN DINÁMICO (Cambia la ruta según la pestaña) */}
          <Link
            to={activeTab === 'venta' ? "/create-ad" : "/create-ad-rent"}
            className="bg-red-700 hover:bg-red-600 px-6 py-2 rounded-lg font-bold transition mb-2"
          >
        ...
        {filteredAds.length === 0 ? (
          <div className="bg-zinc-800 p-12 rounded-2xl border border-zinc-700 text-center">
            <p className="text-zinc-500 text-lg mb-4">
              Aún no tienes ningún vehículo de <span className="font-bold text-white uppercase">{activeTab}</span> publicado.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAds.map((ad: any) => (
              <div
                key={ad.id}
                className="bg-zinc-800 rounded-2xl overflow-hidden border border-zinc-700 hover:border-red-900 transition group relative"
              >
                {/* ETIQUETA VISUAL DEL TIPO DE ANUNCIO */}
                ...
                <div className="p-5">
                  <h3 className="font-bold text-xl mb-1 uppercase truncate">
                  ...
                  <div className="flex items-baseline gap-1 mb-4">
                    ...
                    {/* Añadimos "/ día" si estamos en la pestaña de alquiler */}
                    {activeTab === 'alquiler' && (
                      <span className="text-zinc-500 text-sm font-bold uppercase italic">/ día</span>
                    )}
                  </div>
